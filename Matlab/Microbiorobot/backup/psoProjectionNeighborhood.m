...
swarm(:,1,:) = xIni; % current position
swarm(:,3,:) = xIni; % local best position
swarm(:,2,:) = vIni; % velocity of the swarm
swarm(:,4,1) = 1000000000; % local best value
swarm(:,5,:) = xIni; % global best position
swarm(:,6,1) = 1000000000; % global best (based on the neighborhood of the swarm) value
% [tempValue, tempIndex] = min(swarm(:, 4, 1));
% gbest = swarm(tempIndex, 3, :);
gbestValue = []; % the gather of the gbest values
stopValue = []; % the stop criteria of the iteration
...
for iter = 1 : iteration
    ...
    for i = 1 : np
        ...
        [tempValue, tempIndex] = min(neighborSwarm(:, 4, 1)); % global best position
        if (tempValue < swarm(i,6,1))
            gbest = neighborSwarm(tempIndex,3,:);
        end
        swarm(i,5,:) = gbest;
        swarm(i,6,1) = fun(gbest);
        swarm(i, 2, :) = inertia*swarm(i, 2, :) + correction_factor*rand(1,1,size(xIni,2)).*(swarm(i, 3, :) - swarm(i, 1, :)) + correction_factor*rand(1,1,size(xIni,2)).*(swarm(i,5,:) - swarm(i, 1, :));   % velocity component
    end
    gbestValue(end+1) = min(swarm(:,6,1));
