...
double CostFuncTarget::CellFunction(vector<double>* x, TetrahymenaSwarm swarm)
{
    ...
    vector<vector<double>*>* xCollect = new vector<vector<double>*>;
    vector<vector<double>*>* yCollect = new vector<vector<double>*>;
	...
	{
        vector<double>* xCollectCell = new vector<double>;
        vector<double>* yCollectCell = new vector<double>;
		for (int j = 0; j < (int)swarm.curState->size(); j++)
		{
            ...
            // collect the sate information
            xCollectCell->push_back(state->at(j)->at(0));
            yCollectCell->push_back(state->at(j)->at(1));
		}
        // record state
        xCollect->push_back(xCollectCell);
        yCollect->push_back(yCollectCell);
	}
    
    // calculate the obstacle potential
    double potential = 0;
    if (swarm.potentialType == 1)
    {
        potential = CostFuncTarget::ExponentialPotential(xCollect, yCollect, swarm);
    }
    if (swarm.potentialType == 2)
    {
        potential = CostFuncTarget::ImpulsePotential(xCollect, yCollect, swarm);
    }
