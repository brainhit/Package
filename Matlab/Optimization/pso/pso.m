...
function [xMin,yMin,gbestValue] = pso(fun,np,lb,ub,inertia,correction_factor,iteration,stopIter,x0)
...
%     neighborSwarm = swarm(swarmDist<=threshold,:,:); % find the neighborhood particles
    ...
    % stop criteria
%     if iter > stopIter
%         deltaGbestValue = abs(log(gbestValue(2:end)) - log(gbestValue(1:end-1)));
%         deltaGbestValueLog = deltaGbestValue(deltaGbestValue~=0);
%         if length(deltaGbestValueLog) > 10
%             stopValue = sum(gbestValue(end-9:end));
...
%     end
    if iter > stopIter
        expValue = exp(-gbestValue);
        if length(expValue) > 10
            stopValue = sum(expValue(end-9:end));
            sumDelta = expValue(end-9)-expValue(end);
        else
            stopValue = sum(gbestValue);
            sumDelta = expValue(1)-expValue(end);
        end
        if sumDelta < stopValue/2
            break;
        end
