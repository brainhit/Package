...
function [xMin,yMin] = psoTest(fun,np,lb,ub,inertia,correction_factor,iteration,stopIter,x0)
...
seed = 1; % the seed to find the random number
inputNumSet = load('RandomNumber.txt'); % load the random number set
...
% end
for i = 1:np
    xIniTemp = zeros(1,length(lb));
    vIniTemp = zeros(1,length(lb));
    for j = 1:length(lb)
        xIniTemp(1,j) = ModifyRandomNum(inputNumSet,seed,lb(j),ub(j));
        seed = seed+1;
        vIniTemp(1,j) = ModifyRandomNum(inputNumSet,seed,-abs(ub(j)-lb(j)),abs(ub(j)-lb(j)));
        seed = seed+1;
    end
    xIni = [xIni;xIniTemp];
    vIni = [vIni;vIniTemp];
...
%     neighborSwarm = swarm(swarmDist<=threshold,:,:); % find the neighborhood particles
    ...
    % velocity
    for i = 1:np
        r1 = ModifyRandomNum(inputNumSet,seed,0,1);
        seed = seed+1;
        r2 = ModifyRandomNum(inputNumSet,seed,0,1);
        seed = seed+1;
        swarm(i, 2, :) = inertia*swarm(i, 2, :) + correction_factor*r1.*(swarm(i, 3, :) - swarm(i, 1, :)) + correction_factor*r2.*(gbest - swarm(i, 1, :));   %x velocity component
    end
