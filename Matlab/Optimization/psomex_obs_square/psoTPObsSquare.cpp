...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    //mxArray *fun_in = NULL, *np_in = NULL, *lb_in = NULL, *ub_in = NULL, *inertia_in = NULL, *correction_factor_in = NULL, *iteration_in = NULL, *stopIter_in = NULL, *x0_in = NULL, *obs_in = NULL;
    mxArray *fun_in, *np_in, *lb_in, *ub_in, *inertia_in, *correction_factor_in, *iteration_in, *stopIter_in, *x0_in, *obs_in;
    ...
    obs_in = mxDuplicateArray(prhs[9]);
    ...
    // obstacle info
    int obsNum = (int)obs_val[0];
    swarm.potentialType = (int)obs_val[1];
    swarm.potentialWeightingFactor = obs_val[2];
    for (int i = 0; i < obsNum; i++)
    {
        vector<double>* tmpCenter = new vector<double>;
        tmpCenter->push_back(obs_val[3+7*i+0]);
        tmpCenter->push_back(obs_val[3+7*i+1]);
        swarm.obsCenter->push_back(tmpCenter);
        swarm.obsRadius->push_back(obs_val[3+7*i+2]);
        swarm.obsExpTol->push_back(obs_val[3+7*i+3]);
        swarm.obsExpWeight->push_back(obs_val[3+7*i+4]);
        swarm.obsImpulsePotential->push_back(obs_val[3+7*i+5]);
        swarm.obsImpulseTol->push_back(obs_val[3+7*i+6]);
    }
    ...
    double *xMin_val = new double, *yMin_val = new double;
