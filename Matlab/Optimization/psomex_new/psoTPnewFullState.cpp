...
struct cellParaStruct{
	...
	float cellTarget[40]; // cell target
}cellPara;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	for (int i = 0; i < cellPara.cellNo; i++)
	{
		...
		cellPara.cellTarget[i*4+0] = fun_val[1+2*cellPara.cellNo+3*cellPara.cellNo+i*4+0];
		cellPara.cellTarget[i*4+1] = fun_val[1+2*cellPara.cellNo+3*cellPara.cellNo+i*4+1];
		cellPara.cellTarget[i*4+2] = fun_val[1+2*cellPara.cellNo+3*cellPara.cellNo+i*4+2];
		cellPara.cellTarget[i*4+3] = fun_val[1+2*cellPara.cellNo+3*cellPara.cellNo+i*4+3];
	}
...
float CellFunction(float x[], int xDim, cellParaStruct cell)
{
	// update cell pos and angle based on x values
    ...
    // mod angle to the [-pi,pi] domain
    for (int i = 0; i < cell.cellNo; i++)
    {
        cell.cellState[i*3+2] = fmod(cell.cellState[i*3+2],2*PI);
        if (cell.cellState[i*3+2] > PI)
            cell.cellState[i*3+2] = cell.cellState[i*3+2]-2*PI;
    }
	...
	for (int i = 0; i < cell.cellNo; i++)
	{
		cost += pow(cell.cellState[i*3+0]-cell.cellTarget[i*4+0],2)+\
                pow(cell.cellState[i*3+1]-cell.cellTarget[i*4+1],2)+\
                cell.cellTarget[i*4+3]*pow(cell.cellState[i*3+2]-cell.cellTarget[i*4+2],2);
	}
    cost = sqrt(cost);
